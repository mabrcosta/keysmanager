...
class KeysServiceImpl[TDBIO[_], TDBOut[_]] @Inject()(
  ...
  override def delete[R: _tDBOut: _ownerReader: _keysErrorEither](uid: UUID): Eff[R, Boolean] = {
    for {
      uidOwner <- ask
      keyOpt <- keysDal.findForOwner(uid, uidOwner).execute
      key <- if (keyOpt.isDefined) right(keyOpt.get)
      else left[R, KeysError, Key](KeyNotFound(s"Unable to find key for uid $uid"))
